...
import {searchRequestFootball} from '../utils/voice-helper'
        ...
        getExpressions().then((expressions)=>{
            ...
            if(self.props.recognition){
                self.props.recognition.onresult = function(event) {
                    ...
                    if(result.isFinal){
                    console.log("test " ,result[0].transcript, expressions);
                        var objRequest = searchRequestFootball(result[0].transcript, expressions);
